...
async function warnPlayer() {
    ...
    const reason = await txAdminPrompt({
        ...
        description: 'Warn Grund',
    });
    if (reason === false) return;
    if (!reason.length) {
        return $.notify({ message: '<p class="text-center">Du musst einen Grund angeben!.</p>' }, { type: 'danger' });
    }
    ...
    txAdminAPI({
        ...
        success: function (data) {
            ...
            if (data.success === true) {
                notify.update('type', 'success');
                notify.update('message', 'Spieler Wurde gewarnt.');
...
function banPlayer() {
    ...
    if (!reason.length) {
        $.notify({ message: '<p class="text-center">Dz musst einen Grund angeben!</p>' }, { type: 'danger' });
        return;
    }
    ...
    txAdminAPI({
        ...
        success: function (data) {
            ...
            if (data.success === true) {
                notify.update('type', 'success');
                notify.update('message', 'Spieler wurde gebannt.');
                modPlayer.Modal.hide();
            } else {
                notify.update('type', 'danger');
                notify.update('message', data.error || 'unknown error');
...
async function kickPlayer() {
    ...
    const reason = await txAdminPrompt({
        ...
        title: 'Kick Player',
        description: 'Gebe den Grund an oder lasse das Feld leer (Enter)',
        required: false,
    });
    if (reason === false) return;
    ...
    txAdminAPI({
        ...
        success: function (data) {
            if (data.success === true) {
                notify.update('type', 'success');
                notify.update('message', 'Spieler wurde gekickt.');
            } else {
                notify.update('type', 'danger');
                notify.update('message', data.error || 'unknown error');
            }
